...
int main(int argc, char **argv)
{
    ...
    std::string base64str = base64_encode(plaintext, len);
    std::string plain_str = base64_decode(base64str);
    std::cout << plain_str << std::endl;
    if (strncmp(input.c_str(), plain_str.c_str(), len))
        printf("FAIL: enc/dec failed for \"%s\"   %s\n", input.c_str(), plain_str.c_str());
    else
